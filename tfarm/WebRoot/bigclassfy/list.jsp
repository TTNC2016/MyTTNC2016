		...
		<table align="center">
			...
			<c:choose>
				<c:when test="${not empty list }">
					<c:forEach items="${list}" var="bclassfy" varStatus="status">
						<tr>
							<td><input type="checkbox" name="one"
								value="${bclassfy.fbclass_id}">
							</td>
							<td align="center">${bclassfy.fbclass_id}</td>
							<td align="center">${bclassfy.fbclass_name}</td>
							<td align="center">${bclassfy.create_time}</td>
							<td align="center"><a
								href="findbyid.action?bclassId=${bclassfy.fbclass_id }">修改</a>|<a
								href="deleteOne.action?bclassId=${bclassfy.fbclass_id}">删除</a></td>
						</tr>
					</c:forEach>
				</c:when>
				<c:otherwise>
			...
			<tr>
				...
				<td align="center"><a href="<%=basePath%>/bigclassfy/add.jsp">增加</a>
				</td>
			</tr>
		</table>
